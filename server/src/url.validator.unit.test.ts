...
describe('parseShortenBody', () => {
  ...
  it('throws on a non-http(s) protocol', () => {
    expect(() => parseShortenBody({ originalUrl: 'ftp://example.com' })).toThrow(ZodError)
  })

  it('throws on a malformed URL string', () => {
    expect(() => parseShortenBody({ originalUrl: 'not a url' })).toThrow(ZodError)
  })
...
describe('parseRedirectParams', () => {
  ...
  it('throws on a 17-character shortCode', () => {
    expect(() => parseRedirectParams({ shortCode: 'a'.repeat(17) })).toThrow(ZodError)
  })

  it('throws on a shortCode with non-base62 characters', () => {
    expect(() => parseRedirectParams({ shortCode: 'abc-defg' })).toThrow(ZodError)
  })
